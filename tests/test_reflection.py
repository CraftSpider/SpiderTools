...
def test_command_lang_docs():
    result = reflection.get_undoced("spidertools.command_lang")
    assert len(result) is 0, f"Missing documentation on: {', '.join(map(lambda x: x[0], result))}"


def test_common_docs():
    result = reflection.get_undoced("spidertools.common")
    assert len(result) is 0, f"Missing documentation on: {', '.join(map(lambda x: x[0], result))}"


def test_discord_docs():
    result = reflection.get_undoced("spidertools.discord")
    assert len(result) is 0, f"Missing documentation on: {', '.join(map(lambda x: x[0], result))}"


def test_twitch_docs():
    result = reflection.get_undoced("spidertools.twitch")
    assert len(result) is 0, f"Missing documentation on: {', '.join(map(lambda x: x[0], result))}"


def test_webserver_docs():
    result = reflection.get_undoced("spidertools.webserver")
    assert len(result) is 0, f"Missing documentation on: {', '.join(map(lambda x: x[0], result))}"
...
def test_stub_files():
    ...
    for code, stub in reflection.walk_with_stub_files(".", stub_dir="stubs", skip_dirs=SKIP_DIRS):
...
def test_stub_funcs():
    ...
    for code, stub in reflection.walk_with_modules(".", stub_dir="stubs", skip_dirs=SKIP_DIRS):
